/**
 * Light and dark colours for the Liquid Glass look, following the system's
 ...
 *   applies while the Glass look is on (see web-fixes.ts).
 * - Android: chosen when the app starts.
 *
 * Outside Glass (and when the app didn't start in Glass) the light colour
 * is used as is, so the iOS 6 look never changes.
 */
...
export function themed(name: string, light: string, dark: string): ColorValue {
  ...
  if (!startedInGlass) return light;
...
export function darkModeCss(): string {
  ...
  return `@media (prefers-color-scheme: dark) { html.glass-font { ${vars} } }`;
}
