...
export function Texture({ source, tile, cover = false }: { source: ImageSourcePropType; tile: { width: number; height: number }; cover?: boolean }) {
  // `cover`: one image stretched to fill (a wallpaper) instead of tiles.
  if (cover && Platform.OS !== 'web') return <Image source={source} resizeMode="cover" style={StyleSheet.absoluteFill} />;
  return Platform.OS === 'web' ? <WebTexture source={source} tile={tile} cover={cover} /> : <TiledTexture source={source} tile={tile} />;
}

const noop = () => () => {};

function WebTexture({ source, tile, cover }: { source: ImageSourcePropType; tile: { width: number; height: number }; cover: boolean }) {
  ...
  const background = uri
    ? ({
        ...
        backgroundSize: cover ? 'cover' : `${tile.width}px ${tile.height}px`,
